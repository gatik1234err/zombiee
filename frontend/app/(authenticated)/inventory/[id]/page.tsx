...
export default function APIDetailPage() {
  ...
  const apiId = params?.id as string | undefined
  ...
  useEffect(() => {
    if (!apiId) {
      console.error('API detail page: no apiId from useParams', { params })
    }
  }, [apiId])

  const { data: apiData, isLoading, error } = useQuery<any>({
    queryKey: ['api-detail', apiId],
    queryFn: () => api.getAPIDetail(apiId!),
    enabled: !!apiId,
    retry: false,
  })
  ...
  const quarantineMutation = useMutation({
    mutationFn: () => api.quarantineAPI(apiId!),
    onSuccess: () => {
  ...
  const rescueMutation = useMutation({
    mutationFn: () => api.rescueAPI(apiId!),
    onSuccess: () => {
  ...
  const decommissionMutation = useMutation({
    mutationFn: () => api.decommissionAPI(apiId!),
    onSuccess: () => {
  ...
  const apiDetail = apiData

  if (!apiDetail) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-400 mb-2">
          {error ? `Error: ${error instanceof Error ? error.message : 'Failed to load API details'}` : 'API not found'}
        </p>
        <button
  ...
  const trafficData = apiDetail.traffic_metrics?.slice(-24).map((m: any) => ({
  ...
  const summaryMetrics = [
    { label: 'First Seen', value: formatDate(apiDetail.first_seen), icon: Clock },
    { label: 'Last Seen', value: timeAgo(apiDetail.last_seen), icon: Activity },
    { label: 'Owner', value: apiDetail.owner_team || 'Unassigned', icon: User },
    { label: 'Tags', value: apiDetail.tags?.join(', ') || 'None', icon: Tag },
    { label: 'Domain', value: apiDetail.business_domain || 'N/A', icon: BarChart3 },
    { label: 'Framework', value: apiDetail.framework || 'Unknown', icon: Shield },
  ]
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-card shadow-card p-6">
        <div className="flex items-start justify-between">
          <div className="flex items-start gap-4 min-w-0">
            <RiskGauge score={apiDetail.risk_score} size={80} />
            <div className="min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <span className={`px-2 py-0.5 text-xs font-mono font-bold rounded ${
                  apiDetail.http_method === 'GET' ? 'bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-300' :
                  apiDetail.http_method === 'POST' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300' :
                  apiDetail.http_method === 'PUT' ? 'bg-orange-100 text-orange-700 dark:bg-orange-900/50 dark:text-orange-300' :
                  'bg-red-100 text-red-700 dark:bg-red-900/50 dark:text-red-300'
                }`}>
                  {apiDetail.http_method}
                </span>
                <span className="font-mono text-sm text-gray-500 dark:text-gray-300 truncate max-w-[400px]">
                  {apiDetail.endpoint_path}
                </span>
                <button
                  onClick={() => copyToClipboard(apiDetail.endpoint_path)}
              ...
              <div className="flex items-center gap-2">
                <StatusBadge status={apiDetail.status} />
                <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 capitalize">
                  {apiDetail.environment}
                </span>
                <span className="text-xs text-gray-400">{apiDetail.protocol} &middot; {apiDetail.tls_version}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {(apiDetail.status === 'zombie' || apiDetail.status === 'orphaned') && (
              <button
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-card shadow-card p-5">
            ...
            <div className="space-y-3">
              {[
                { label: 'Authentication', score: Math.max(0, apiDetail.risk_score - 20) },
                { label: 'Authorization', score: Math.max(0, apiDetail.risk_score - 15) },
                { label: 'Encryption', score: Math.max(10, 100 - apiDetail.risk_score) },
                { label: 'Rate Limiting', score: Math.max(0, 80 - apiDetail.risk_score) },
                { label: 'Input Validation', score: Math.max(0, apiDetail.risk_score - 10) },
              ].map((item) => (
                <div key={item.label}>
      ...
      {activeTab === 'security' && (
        <div className="bg-white dark:bg-gray-800 rounded-card shadow-card p-5">
          ...
          <div className="space-y-3">
            {apiDetail.security_findings?.length > 0 ? apiDetail.security_findings.map((finding: any) => (
